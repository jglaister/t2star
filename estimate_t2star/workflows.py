...
def create_mtr_workflow(scan_directory: str, patient_id: str = None, scan_id: str = None, reorient: str = 'RAI', split_mton_flag = False, num_threads: int = 1) -> pe.Workflow:
    ...
    def identity(input_file):
        return input_file

    mton_node = pe.Node(util.Function(input_names=["input_file"], output_names=["output_file"], function=identity), name='mton_node')
    mtoff_node = pe.Node(util.Function(input_names=["input_file"], output_names=["output_file"], function=identity), name='mtoff_node')
    ...
    wf.connect(input_node, 'mton_file ', mton_node, 'input_file')
    wf.connect(input_node, 'mtoff_file ', mtoff_node, 'input_file')
    ...
    if reorient is not None:
        ...
        wf.connect(mton_node, 'output_file', reorient_mton_to_target, 'in_file')
        ...
        wf.connect(mtoff_node, 'output_file', reorient_mtoff_to_target, 'in_file')
    ...
    if reorient is not None:
        ...
        wf.connect(mtoff_node, 'output_file', affine_reg_to_target, 'moving_image')
    ...
    if reorient is not None:
        ...
        wf.connect(mton_node, 'output_file', transform_mton, 'input_image')
